...
def check_file(md_path: Path, min_valid_pct: float = 95.0,
    ...
    # Недоразложенные элементы списков тех же секций (замечание
    # владельца: в FLD-29 разложен только первый блок, ФИАС-перечень
    # с вложенным условием остался строкой li): ≥2 «если» в одном
    # элементе — ветвление не довскрыто.
    ...
    for zm in zones_re.finditer(text):
        ...
        for ln in zone_txt.splitlines():
            if not re.match(r"\s*-\s", ln) or _attr_line_re.match(ln):
                continue
            conds = _cond_count(ln)
            if conds >= 2:
                ok = False
                report.append(
                    f"сплющенный элемент списка ({zm.group(0).strip()}): "
