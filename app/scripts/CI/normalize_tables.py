...
def check_file(md_path: Path, min_valid_pct: float = 95.0,
    ...
    # вложенные условия ячейки источника (<ul> в <ul>, если/то/иначе)
    # выпрямлены в однострочную простыню — дерево ветвления нечитаемо,
    # реализация по такой логике недостоверна (тот же класс, что
    # «выпрямление меняет смысл ветвления» у Поведения). Ловится по
    # карточке: ≥2 «если» в значении атрибута ОДНОЙ строкой; значение,
    # разложенное подсписком, в строке метки текста не несёт и не
    # флагается (конструкция, не запрет). Кавычечные литералы и
    # скобочные вставки — текст автора, из подсчёта исключены.
    for m in re.finditer(
            r"^\s*-\s+\*\*(Логика установки значения|Видимость|"
            r"Источник списка|По умолчанию)[^\n:]*:\*\*(.+)$",
            text, re.M):
        tail = re.sub(r'"[^"\n]*"|«[^»\n]*»|\([^()\n]*\)', " ",
                      m.group(2))
        conds = len(re.findall(r"\bесли\b", tail, re.I))
        if conds >= 2:
            ok = False
            report.append(
                f"сплющенный атрибут «{m.group(1)}»: {conds} условия "
