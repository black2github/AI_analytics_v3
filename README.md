...
|-----------|-------|----------|
| **API-интерфейс** | `app/main.py`<br>`app/routes/analyze.py`<br>`app/routes/jira.py`<br>`app/routes/loader.py`<br>`app/routes/template_analysis.py`<br>`app/routes/services.py`<br>`app/routes/health.py`<br>`app/routes/info.py`<br>`app/routes/test_context.py`<br>`app/routes/logging_control.py` | FastAPI приложение и все API эндпоинты |
| **Конфигурация** | `app/config.py`<br>`app/data/services.json`<br>`app/data/templates.json`<br>`app/data/features.json`<br>`.env`<br>`requirements.txt` | Настройки приложения, справочники и зависимости |
| **Работа с эмбеддингами и хранилищем** | `app/embedding_store.py`<br>`app/semantic_search.py`<br>`app/services/context_builder.py`<br>`app/services/document_service.py` | Векторное хранилище, поиск и управление документами |
| **Интеграция с Confluence** | `app/confluence_loader.py`<br>`app/content_extractor.py`<br>`app/filter_all_fragments.py`<br>`app/filter_approved_fragments.py`<br>`app/history_cleaner.py`<br>`app/style_utils.py` | Загрузка и обработка контента из Confluence |
| **Интеграция с Jira** | `app/jira_loader.py` | Извлечение данных из задач Jira |
| **LLM-анализ** | `app/llm_interface.py`<br>`app/rag_pipeline.py`<br>`app/services/analysis_service.py`<br>`app/services/template_type_analysis.py`<br>`page_prompt_template.txt`<br>`template-analysis-prompt.txt` | Интерфейсы к LLM и алгоритмы анализа |
| **Сервисы и реестры** | `app/service_registry.py`<br>`app/template_registry.py` | Управление сервисами и шаблонами |
| **Утилиты** | `app/logging_config.py`<br>`app/logging_utils.py` | Вспомогательные функции и настройка логирования |
